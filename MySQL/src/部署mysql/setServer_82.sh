...
replica_load_tmpdir             =${INS_BASE_DIR}/tmp
...
#备份用户
CREATE USER 'bkpuser'@'%' IDENTIFIED BY "n#CJ^f&i";
GRANT SELECT,SHOW VIEW,TRIGGER,LOCK TABLES,RELOAD,PROCESS,SUPER ON *.* TO  'bkpuser'@'%' WITH GRANT OPTION;  
#复制用户
CREATE USER 'replic'@'%' IDENTIFIED BY "4%REplic";
GRANT REPLICATION SLAVE, REPLICATION CLIENT ON *.* TO 'replic'@'%' WITH GRANT OPTION;  
#inception用户
CREATE USER 'inception'@'%' identified by 'fF&w@tw5qZK1WRvi';
GRANT SELECT, INSERT, UPDATE, DELETE, CREATE, DROP, PROCESS, INDEX, ALTER, SUPER, REPLICATION SLAVE, REPLICATION CLIENT, TRIGGER ON *.* TO 'inception'@'%';
flush privileges;
shutdown;
EOF
[ $? == "0" ] && echo " 密码修改成功!"
