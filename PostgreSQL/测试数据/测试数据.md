...
```sql
create table test_t (id int,info text);
insert into test_t select seq,seq||'abcdefghijklmn' from generate_series(1,10000) as seq;
```

一般人会用limit语句来获取数据，但是这样随机性不好，示例如下

```sql
postgres=# select * from test_t limit 10;
 id |       info
----+------------------
  1 | 1abcdefghijklmn
  2 | 2abcdefghijklmn
...
-- 效率不高，会全表扫描
postgres=# explain analyze select * from test_t limit 10;
                                                  QUERY PLAN
--------------------------------------------------------------------------------------------------------------
 Limit  (cost=0.00..0.16 rows=10 width=22) (actual time=0.015..0.017 rows=10 loops=1)
   ->  Seq Scan on test_t  (cost=0.00..164.00 rows=10000 width=22) (actual time=0.014..0.015 rows=10 loops=1)
...
```sql
postgres=# select * from test_t where id%1000=1;
  id  |        info
------+--------------------
    1 | 1abcdefghijklmn
 1001 | 1001abcdefghijklmn
 2001 | 2001abcdefghijklmn
 3001 | 3001abcdefghijklmn
 4001 | 4001abcdefghijklmn
 5001 | 5001abcdefghijklmn
 6001 | 6001abcdefghijklmn
 7001 | 7001abcdefghijklmn
 8001 | 8001abcdefghijklmn
 9001 | 9001abcdefghijklmn
(10 rows)

-- 效率不高，会全表扫描
postgres=# explain analyze select * from test_t where id%1000=1;
                                             QUERY PLAN
-----------------------------------------------------------------------------------------------------
 Seq Scan on test_t  (cost=0.00..214.00 rows=50 width=22) (actual time=0.017..0.885 rows=10 loops=1)
   Filter: ((id % 1000) = 1)
...
PostgreSQL提供了数据抽样的语法，可以直接抽取大表的数据：

```sql
select * from test_t tablesample system(0.9);
```

tablesample system(0.9)的含义是随机找一些数据块进行抽样，抽样比例是0.9%
